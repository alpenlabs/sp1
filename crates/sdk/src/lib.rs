...
mod tests {
    ...
    fn test_generate_sect_proof_step_wise() {
        utils::setup_logger();
        // IRL upto compressed proof is generated by SP1 cluster
        // because this is the heavy computation part
        let client = ProverClient::builder().cpu().build();
        ...
        println!("proof public values {:?}", proof.public_values);

        // Now use the compressed proof returned by SP1 Cluster
        // to generate wrapped proof (only sect witness) locally
        // This can't be offloaded to SP1 cluster now because
        // cluster doesn't handle forked code
